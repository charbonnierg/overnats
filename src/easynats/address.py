...
class Placeholders(Generic[ParamsT]):
    ...
    def from_subject(
        ...
        for match in list(pattern.finditer(sanitized_subject)):
            ...
            # Append placeholder
            pos = (
                subject.replace("...", syntax.match_one)
                .split(".")
                .index(placeholder.replace("...", syntax.match_one))
            )
            if is_wildcard:
