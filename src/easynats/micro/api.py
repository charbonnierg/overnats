...
InternalHandler: TypeAlias = Callable[[Msg], Awaitable[None]]
...
def get_internal_subject(
    verb: ServiceVerb, service: str | None = None, id: str | None = None
) -> str:
...
def get_internal_subjects(
    verb: ServiceVerb, id: str, config: ServiceConfig
) -> list[str]:
    """Get the internal subjects for a verb."""
    return [
        get_internal_subject(verb),
        get_internal_subject(verb, config.name),
        get_internal_subject(verb, config.name, id),
    ]
...
class Service:
    ...
    def _create_handler(self, endpoint: Endpoint) -> InternalHandler:
        """Handle a message."""
